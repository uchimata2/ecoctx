...
single-phase cost is 10,129 plus 32,814, or 42,943 bytes.
...
`tools/selftest.py`. Two more are packaging, in `.claude-plugin/`. The seven checkers in `tools/` check
this repository and are not part of an install - one fails when a chart in `assets/` draws a figure this
README does not state, another when the prose here picks up a habit it does not have, a third when the
three files that state the version stop agreeing with each other.
...
Version `rubric 3, revision 0`, stated in `skills/ecoctx/SKILL.md`. The rubric number says whether a
phase 1 and the phase 2 grading it ran under the same rubric. `plugin.json` renders it `3.0.0` because
